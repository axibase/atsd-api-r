...
In this demo we fetch a time-series from ATSD.
Then we forecast this time-series with help of functions from
**stats** and **forecast** R packages.
Then we compare the forecasts with a forecast retrieved from ATSD and with
real data for the same period.
Finally we save all forecasts in ATSD.
...
Then we can set connection with ATSD server:
...
And we will forecast for the last two days March 31 and April 1.
...
First of all we retrieve 2 days forecast from ATSD.
...
gives slightly better result then Holt Winters implemented in ATSD.
